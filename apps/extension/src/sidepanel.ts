...
const STORAGE_PREFERENCES = "neptune.preferences.v1";
const STORAGE_MESSAGES = "neptune.messages.v1";
const STORAGE_AUDIT = "neptune.audit.v1";
...
let pendingApproval: Mission | null = null;
let blockedMission: Mission | null = null;
let blockedMessage = "";
...
async function initialize(): Promise<void> {
  const stored = await chrome.storage.local.get([STORAGE_PREFERENCES, STORAGE_MESSAGES, STORAGE_AUDIT]);
  ...
  if (!preferences.onboardingComplete) {
    ...
    addMessage("assistant", `Bonjour ${preferences.preferredName || ""}. Je suis prêt. Que faisons-nous ?`);
  }
}
...
async function handleClick(event: Event): Promise<void> {
  ...
  switch (action) {
    ...
    case "select-voice":
      ...
      break;
    case "preview-voice":
      speak(`Bonjour. Je suis Neptune. Voici un aperçu de ma voix.`, value);
      break;
    case "select-trust":
    ...
    case "approve":
      ...
      break;
    case "deny":
      await stopMission("Action refusée. La mission a été arrêtée.");
      break;
    case "resume":
      if (blockedMission) await executeMission(blockedMission, false);
      break;
    case "grant-and-resume":
      if (await requestSiteAccess() && blockedMission) await executeMission(blockedMission, blockedMission.currentIndex === 0);
      break;
...
async function nextOnboardingStep(): Promise<void> {
  ...
  await savePreferences();
  addMessage("assistant", `Configuration terminée. ${preferences.preferredName}, dites-moi simplement votre objectif.`);
  render();
  speak(`Configuration terminée. ${preferences.preferredName}, dites-moi simplement votre objectif.`);
}
...
async function submitMessage(text: string): Promise<void> {
  ...
  if (blockedMission && /^(continue|continuer|j'ai terminé|c'est fait|reprends)/i.test(clean)) {
    await executeMission(blockedMission, false);
    return;
  }
  ...
  try {
    if (isBrowserTask(clean)) {
      const reply = await planBrowserTask(provider, preferences.preferredName, preferences.trustLevel, clean, undefined, abortController.signal);
      typing = false;
      addMessage("assistant", reply.text, reply.actions.some(needsApproval) ? "permission" : "normal");
      if (reply.actions.length === 0) return;
      mission = {
      ...
      detailsOpen = true;
      appendAudit("MISSION_PLANNED", `${reply.actions.length} action(s) : ${clean}`);
      await executeMission(mission, true);
      return;
    }
...
async function executeMission(source: Mission, startNew: boolean): Promise<void> {
  if (!preferences.siteAccessGranted) {
    blockedMission = source;
    blockedMessage = `${preferences.preferredName}, Chrome doit autoriser Neptune à agir sur les sites. Cliquez sur « Autoriser et reprendre ».`;
    orbState = "blocked";
    render();
    return;
  }
  ...
  try {
    ...
    for (let index = working.currentIndex; index < working.actions.length; index += 1) {
      ...
      if (needsApproval(action) && !working.approved) {
        action.status = "blocked";
        pendingApproval = cloneMission(working);
        mission = cloneMission(working);
        orbState = "permission";
      ...
      const response = await sendRuntime({
        ...
        approved: working.approved || !needsApproval(action)
      });
    ...
    mission = cloneMission(working);
    orbState = "thinking";
    const summary = await summarizeMission(working);
    addMessage("assistant", summary);
    speak(summary);
    appendAudit("MISSION_COMPLETED", working.goal);
  } catch (error) {
    const current = working.actions[working.currentIndex];
    if (current) current.status = "failed";
    mission = cloneMission(working);
    blockedMission = cloneMission(working);
    blockedMessage = describeBrowserError(error as BrowserError | undefined);
    orbState = "blocked";
    addMessage("assistant", blockedMessage, "warning");
    speak(blockedMessage);
    appendAudit("MISSION_BLOCKED", blockedMessage);
  } finally {
    busy = false;
    render();
  }
...
async function summarizeMission(completed: Mission): Promise<string> {
  const provider = await currentProviderConfig();
  if (!provider) return "Mission terminée.";
  const lastUseful = [...completed.results].reverse().find((item) => item.result && typeof item.result === "object");
  if (!lastUseful) return "Mission terminée. Les actions prévues ont été exécutées.";
  try {
    return await askIntelligence(provider, preferences.preferredName, [{
      role: "user",
      content: `Objectif : ${completed.goal}\nRésultat navigateur : ${JSON.stringify(lastUseful.result).slice(0, 14_000)}\nRéponds avec une synthèse utile, factuelle et courte. Ne prétends rien qui ne figure pas dans le résultat.`
    }]);
  } catch {
    return "Mission terminée. Le résultat est disponible dans les détails de la mission.";
  }
}

async function approveMission(): Promise<void> {
  if (!pendingApproval) return;
  const approved = cloneMission(pendingApproval);
  approved.approved = true;
  const current = approved.actions[approved.currentIndex];
  if (current) current.status = "pending";
  pendingApproval = null;
  mission = approved;
  appendAudit("MISSION_APPROVED", current?.label ?? approved.goal);
...
function toggleListening(): void {
  ...
  recognition.onerror = (event) => {
    if (event.error !== "aborted" && event.error !== "no-speech") {
      addMessage("assistant", voiceErrorMessage(event.error), "warning");
    }
  };
  recognition.onend = () => {
    recognitionRunning = false;
    if (listeningWanted && preferences.wakeWordEnabled) {
      window.setTimeout(() => {
...
function speak(text: string, voiceURI = preferences.voiceURI): void {
  ...
  const previousState = orbState;
  utterance.onstart = () => { orbState = "speaking"; render(); };
  utterance.onend = () => { if (orbState === "speaking") orbState = previousState === "listening" ? "listening" : "idle"; render(); };
  utterance.onerror = () => { if (orbState === "speaking") orbState = "idle"; render(); };
...
function renderOnboarding(): string {
  ...
  return `<section class="onboarding">
    ...
    <div class="onboarding-body">
      ${renderOrb(true)}
      <div class="onboarding-copy">${onboardingStepMarkup(step)}</div>
    </div>
    <footer class="onboarding-actions">
      <button class="ghost-button" data-action="onboarding-back" ${step === 0 ? "disabled" : ""}>Retour</button>
      <button class="primary-button" data-action="onboarding-next" ${canContinueOnboarding() ? "" : "disabled"}>${step === ONBOARDING_LAST_STEP ? "Commencer avec Neptune" : "Continuer"}</button>
    </footer>
  </section>`;
}

function onboardingStepMarkup(step: number): string {
  if (step === 0) return `<p class="eyebrow">PREMIER ÉCHANGE</p><h2>Comment dois-je vous appeler ?</h2><p>J’utiliserai votre prénom uniquement lorsque l’échange le justifie, notamment en cas de blocage ou de validation importante.</p><div class="field"><label for="preferred-name">Votre prénom</label><input id="preferred-name" class="input" value="${escapeAttribute(preferences.preferredName)}" placeholder="Johan" autofocus /></div>`;

  if (step === 1) return `<p class="eyebrow">PERSONNALITÉ VOCALE</p><h2>Choisissez ma voix</h2><p>Pré-écoutez les voix françaises disponibles sur votre ordinateur. Vous pourrez la changer plus tard.</p><div class="choice-grid">${voiceOptionsMarkup()}</div>`;
  ...
  if (step === 3) return `<p class="eyebrow">INTELLIGENCE</p><h2>Comment souhaitez-vous que je réfléchisse ?</h2><p>Local pour la confidentialité, cloud pour davantage de puissance. Aucun jeton technique Neptune n’est demandé.</p><div class="choice-grid">${providerOptionsMarkup()}</div>${providerConfigurationMarkup()}`;

  if (step === 4) return `<p class="eyebrow">ACTIVATION VOCALE</p><h2>Comment souhaitez-vous m’appeler ?</h2><p>Le mot d’activation fonctionne lorsque le panneau Neptune est ouvert. Le bouton micro reste toujours disponible.</p><div class="choice-grid">
    ${choiceButton("select-wake-word", "Neptune", "Neptune", "Activation courte et naturelle", preferences.wakeWordEnabled && preferences.wakeWord === "Neptune")}
    ${choiceButton("select-wake-word", "OK Neptune", "OK Neptune", "Réduit les activations accidentelles", preferences.wakeWordEnabled && preferences.wakeWord === "OK Neptune")}
    ${choiceButton("disable-wake-word", "", "Bouton micro uniquement", "Aucune écoute continue", !preferences.wakeWordEnabled)}
  </div>`;

  if (step === 5) return `<p class="eyebrow">ACCÈS NAVIGATEUR</p><h2>Autorisez mon onglet de travail</h2><p>Chaque mission s’exécute dans un nouvel onglet Neptune. Votre onglet actuel n’est jamais réutilisé par défaut.</p><div class="notice ${preferences.siteAccessGranted ? "success" : "warning"}">${preferences.siteAccessGranted ? "Accès accordé. Neptune peut lire et agir sur les sites demandés." : "Chrome affichera une confirmation unique pour autoriser l’accès aux sites."}</div><div class="inline-actions"><button class="primary-button" data-action="grant-site-access">${preferences.siteAccessGranted ? "Vérifier l’autorisation" : "Autoriser les sites"}</button></div>`;

  return `<p class="eyebrow">PRÊT À COMMENCER</p><h2>Votre Neptune est configuré</h2><p>Essayez : « ${preferences.wakeWordEnabled ? `${preferences.wakeWord}, ` : ""}ouvre Le Bon Coin et lis la page d’accueil. »</p><div class="notice success">Intelligence : ${providerLabel(preferences.providerId)} · Confiance : ${trustLabel(preferences.trustLevel)} · Voix : ${escapeHtml(selectedVoice()?.name ?? "système")}</div>`;
}
...
function renderAssistant(): string {
  return `<section class="shell">
    <header class="topbar">
      ...
      <div class="brand-copy"><p class="eyebrow">NEPTUNE</p><h1>Assistant navigateur</h1></div>
      <button class="icon-button" data-action="open-settings" title="Réglages">⚙</button>
    </header>
    <section class="assistant">
      <div class="assistant-head"><span class="state-label">${stateLabel()}</span><div class="inline-actions"><button class="ghost-button" data-action="toggle-details">Mission</button><button class="danger-button" data-action="stop">Arrêter</button></div></div>
      <div id="conversation" class="conversation-stage">
        <div class="mini-orb-wrap">${renderOrb(false)}</div>
        <div class="messages">${messages.map(messageMarkup).join("")}${typing ? typingMarkup() : ""}</div>
        ${pendingApproval ? approvalMarkup() : ""}
        ${blockedMission ? blockedMarkup() : ""}
        ${detailsOpen ? missionDetailsMarkup() : ""}
      </div>
      <div class="composer-wrap">
        <form id="composer" class="composer">
          <button type="button" class="micro-button ${listeningWanted ? "active" : ""}" data-action="toggle-micro" title="${listeningWanted ? "Arrêter l’écoute" : "Parler à Neptune"}">●</button>
          <textarea id="draft" placeholder="Écrivez ou dites votre objectif…" rows="2">${escapeHtml(draft)}</textarea>
          <button type="submit" class="send-button" data-action="send" ${busy ? "disabled" : ""}>➤</button>
        </form>
        <div class="composer-hint"><span>${preferences.wakeWordEnabled ? `Dites « ${preferences.wakeWord} » après avoir activé le micro.` : "Bouton micro ou texte."}</span><span>${providerLabel(preferences.providerId)}</span></div>
      </div>
    </section>
...
function voiceOptionsMarkup(): string {
  ...
  return list.map((voice) => `<div class="choice-card ${preferences.voiceURI === voice.voiceURI ? "selected" : ""}">
    <button class="choice-card" data-action="select-voice" data-value="${escapeAttribute(voice.voiceURI)}"><span class="choice-title-row"><span class="choice-title">${escapeHtml(voice.name)}</span><span class="choice-badge">${escapeHtml(voice.lang)}</span></span><span class="choice-description">${voice.localService ? "Voix locale du système" : "Voix fournie par Chrome"}</span></button>
    <div class="inline-actions"><button class="ghost-button" data-action="preview-voice" data-value="${escapeAttribute(voice.voiceURI)}">Pré-écouter</button></div>
  </div>`).join("");
}
...
function providerConfigurationMarkup(): string {
  if (preferences.providerId === "chrome-local") {
    ...
    return `<div class="notice ${chromeAiStatus === "available" ? "success" : chromeAiStatus === "unavailable" ? "warning" : ""}">${status}. Le modèle est géré par Chrome et aucune donnée n’est envoyée à un fournisseur.</div>${modelProgress > 0 && modelProgress < 100 ? `<div class="download-progress"><span style="width:${modelProgress}%"></span></div>` : ""}<div class="inline-actions"><button class="primary-button" data-action="prepare-local-ai" ${busy || chromeAiStatus === "unavailable" ? "disabled" : ""}>${chromeAiStatus === "available" ? "Réinitialiser le modèle" : `Télécharger et utiliser${modelProgress ? ` · ${modelProgress}%` : ""}`}</button></div>`;
  }
  return `<div class="field"><label for="provider-endpoint">Adresse API</label><input id="provider-endpoint" class="input" value="${escapeAttribute(preferences.providerId === "mammouth" ? "https://api.mammouth.ai/v1" : preferences.endpoint)}" ${preferences.providerId === "mammouth" ? "readonly" : ""} /></div><div class="field"><label for="provider-model">Modèle</label><input id="provider-model" class="input" value="${escapeAttribute(preferences.model || (preferences.providerId === "mammouth" ? "mammouth-recommended" : ""))}" placeholder="mammouth-recommended" /></div><div class="field"><label for="provider-secret">Clé API ${providerSecretSaved ? "— enregistrée et chiffrée" : ""}</label><input id="provider-secret" class="input" type="password" value="${escapeAttribute(secretDraft)}" placeholder="Collez votre clé API" autocomplete="off" /></div><div class="inline-actions"><button class="primary-button" data-action="save-provider-secret">Enregistrer la clé</button>${providerSecretSaved ? `<button class="ghost-button" data-action="test-provider">Tester</button><button class="danger-button" data-action="delete-provider-secret">Supprimer</button>` : ""}</div>`;
}

function approvalMarkup(): string {
  const action = pendingApproval?.actions[pendingApproval.currentIndex];
  return `<section class="intervention permission"><strong>Autorisation requise</strong><p>${escapeHtml(action?.label ?? "Action externe")}</p><div class="inline-actions"><button class="primary-button" data-action="approve">Autoriser et continuer</button><button class="secondary-button" data-action="deny">Refuser</button></div></section>`;
}

function blockedMarkup(): string {
  const permissionMissing = !preferences.siteAccessGranted;
  return `<section class="intervention blocked"><strong>Intervention nécessaire</strong><p>${escapeHtml(blockedMessage)}</p><div class="inline-actions">${permissionMissing ? `<button class="primary-button" data-action="grant-and-resume">Autoriser et reprendre</button>` : `<button class="primary-button" data-action="resume">J’ai terminé, continuer</button>`}<button class="secondary-button" data-action="deny">Arrêter</button></div></section>`;
}

function missionDetailsMarkup(): string {
  return `<section class="intervention"><strong>Détails de la mission</strong>${mission ? `<p>${escapeHtml(mission.goal)}</p><div class="audit-list">${mission.actions.map((action, index) => `<div class="audit-item"><time>${index + 1} · ${escapeHtml(action.status)}</time>${escapeHtml(action.label)}<br><small>${escapeHtml(action.type)} · ${escapeHtml(action.risk)}</small></div>`).join("")}</div>` : `<p>Aucune mission en cours.</p>`}</section>`;
}

function settingsMarkup(): string {
  return `<div class="modal-backdrop"><section class="modal"><header class="modal-head"><div><p class="eyebrow">PRÉFÉRENCES</p><h2>Configurer Neptune</h2></div><button class="icon-button" data-action="close-settings">×</button></header>
    <div class="settings-section"><h3>Identité et voix</h3><div class="field"><label for="settings-name">Votre prénom</label><input id="settings-name" class="input" value="${escapeAttribute(preferences.preferredName)}" /></div><div class="field"><label for="settings-voice">Voix</label><select id="settings-voice" class="select">${voices.map((voice) => `<option value="${escapeAttribute(voice.voiceURI)}" ${voice.voiceURI === preferences.voiceURI ? "selected" : ""}>${escapeHtml(voice.name)} · ${escapeHtml(voice.lang)}</option>`).join("")}</select></div></div>
    <div class="settings-section"><h3>Intelligence</h3><p>${providerLabel(preferences.providerId)} · ${escapeHtml(preferences.model || "modèle local")}</p><div class="inline-actions"><button class="ghost-button" data-action="reset-onboarding">Reconfigurer étape par étape</button><button class="ghost-button" data-action="test-provider">Tester le moteur</button></div></div>
    <div class="settings-section"><h3>Données locales</h3><p>${messages.length} message(s) et ${audit.length} événement(s) conservés dans ce profil Chrome.</p><div class="inline-actions"><button class="ghost-button" data-action="clear-history">Effacer la conversation</button><button class="ghost-button" data-action="clear-audit">Effacer le journal</button></div><div class="audit-list">${audit.slice(-12).reverse().map((entry) => `<div class="audit-item"><time>${formatTime(entry.occurredAt)} · ${escapeHtml(entry.type)}</time>${escapeHtml(entry.detail)}</div>`).join("")}</div></div>
    <div class="settings-section"><h3>Sécurité</h3><p>Neptune refuse les paiements, achats, suppressions, mots de passe, signatures et contournements de protections.</p></div>
  </section></div>`;
}
...
function stateLabel(): string {
  return ({ idle: "Prêt", listening: "Écoute", thinking: "Réflexion", executing: "Exécution", speaking: "Réponse", permission: "Autorisation requise", blocked: "Intervention nécessaire", error: "Erreur" } satisfies Record<OrbState, string>)[orbState];
}

function onboardingSpeech(step: number): string {
  return [
    "Comment dois-je vous appeler ?",
    "Choisissez ma voix. Vous pouvez pré-écouter chaque proposition.",
    "Quel niveau de confiance m’accordez-vous ?",
    "Choisissez mon moteur d’intelligence.",
    "Choisissez mon mot d’activation.",
    "Autorisez mon onglet de travail dans Chrome.",
...
function preferredVoice(): SpeechSynthesisVoice | undefined {
  return voices.find((voice) => voice.lang.toLocaleLowerCase("fr-FR").startsWith("fr") && voice.localService)
    ?? voices.find((voice) => voice.lang.toLocaleLowerCase("fr-FR").startsWith("fr"))
    ?? voices[0];
}
...
function describeBrowserError(error?: BrowserError): string {
  ...
  switch (error.code) {
    case "HUMAN_VERIFICATION": return `${preferences.preferredName}, le site demande une vérification humaine. Effectuez-la dans l’onglet Neptune, puis cliquez sur « J’ai terminé, continuer ».`;
    ...
    case "PAGE_PERMISSION": return `${preferences.preferredName}, Chrome a refusé l’accès à cette page. Vérifiez les autorisations de l’extension puis réessayez.`;
    case "TARGET_NOT_FOUND": return `${preferences.preferredName}, l’élément attendu n’est plus visible. La page a probablement changé.`;
...
function cloneMission(value: Mission): Mission {
  return { ...value, actions: value.actions.map((action) => ({ ...action })), results: [...value.results] };
}
...
function voiceErrorMessage(error: string): string {
  ...
  return messages[error] ?? `Erreur de reconnaissance vocale : ${error}`;
}
