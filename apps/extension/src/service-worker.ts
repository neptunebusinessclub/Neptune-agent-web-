...
type ExtensionRequest =
  ...
  | { type: "BACKGROUND_WAKE_TRANSCRIPT"; transcript: string; wakeWord?: string; occurredAt?: string }
  | { type: "WAKE_DOCUMENT_STATUS"; status: string; error?: string; occurredAt?: string };
...
chrome.windows.onRemoved.addListener((windowId) => {
  void clearAssistantWindowIfMatches(windowId);
});
...
async function openAssistantWindow(): Promise<number | undefined> {
  ...
  const created = await chrome.windows.create({
    url: chrome.runtime.getURL(`${SIDEPANEL_PATH}?mode=voice`),
    type: "popup",
    width: 500,
    height: 780,
    focused: true
...
async function getWakeStatus(): Promise<WakeStatus> {
  ...
  const value = stored[WAKE_STATUS_STORAGE_KEY] as WakeStatus | undefined;
  return value ?? { status: "stopped", updatedAt: new Date(0).toISOString() };
}
